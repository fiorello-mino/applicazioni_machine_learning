...
for ind = 1:K
    ...
    try
        ...
        %
        % SUBSTITUTE SVM WITH TREE BAGGER
        % Mdl = TreeBagger(NumTrees, TrainData, TrainLabel)
        % The function fitcsvm is substituted by TreeBagger
        % We have different input parameters, the most important being
        % the number of trees to be grown
        %
        % REF: https://it.mathworks.com/help/stats/treebagger.html
        %

        nTrees = 10; % Try to increase the number of trees to improve
                     % the results -> 50 -> 100 -> 500
        trained_model = TreeBagger(nTrees, pc_tr_data, temp_train_labels, ...
    ...
    for subject = 1:size(temp_test_data, 1)
        ...
        if size(te_data,2) == 0
           ...
           try
                ...
                %
                % SUBSTITUTE SVM WITH BAGGED TREES
                % The function is the same: predict
                % We only change the input argument
                %
                
                class = predict(trained_model, pc_te_data)';
                
                %
                % WARNING! In this case, the result of the classification
                % (output of the function 'predict') is a cell array, 
                % so it must be converted into a number
